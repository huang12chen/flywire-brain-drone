/* evaluate.js — 确定性蒙特卡洛实验矩阵评估器（A/B/C/D 四组）
 * ============================================================================
 * 用法：node web/evaluate.js
 * 输出：web/results.json（全部格子的 mean/std 明细与样本量）+ 控制台 Markdown 表格
 *
 * 设计要点（严格遵循任务口径）：
 *   1) 全程禁用 Math.random —— 自实现 mulberry32 种子化 PRNG；场景采样与脉冲噪声
 *      各用一条独立随机流（都由同一 seed 派生），因此所有格子共享同一组种子时构成
 *      「共同随机数」配对比较，且逐格可复现。
 *   2) snn_runtime.encode() 原本内部调用 Math.random；该文件已打「唯一一处」补丁：
 *      (this.rng || Math.random)()。默认行为完全不变；本评估器通过 rt.rng = fn 注入
 *      种子化噪声源，实现确定性。
 *   3) 传感器物理、标签口径、指标定义与 train_snn.py 完全一致（见下各函数注释）。
 *
 ...
 * （受 accel / vmax 限制），dt=1ms 积分，全程完整二次型最近点扫掠：
 * 「任一步扫掠最近距 < 半径和（撞）」或「全程扫掠完成无碰撞（安全）」。
 * latencyMs == null 表示 GF 未放电（载体不动，作基线用）。 */
function planarEscapeOutcome(st, latencyMs, escapeDir3, body) {
  ...
  for (let t = 1; t <= T_MAX_MS; t++) {
    ...
    // 步内扫掠判距：本步相对线段到原点的二次型最近点（含端点；防 1ms 步内凹谷穿透）
    ...
    // 【v5 P5①】完整二次型最近点判定：全程逐步取最小最近距，不用"局部距离回升"提前判安全
    //   （相对航迹受机动加速度为分段二次曲线，距离²可呈双谷；首谷回升即返回会漏掉第二段接近 → 误报安全）
    if (sweptMin < minDist) minDist = sweptMin;
  }
  return { safe: minDist > rSum, minDist, reason: 'escaped' };   // 全程扫掠完成且无碰撞 = 真安全
}
...
function main() {
  const t0 = Date.now();
  console.log('确定性蒙特卡洛实验矩阵评估器 —— 果蝇逃逸 SNN');
  ...
  console.log(`\n已写出：${outPath}（耗时 ${((Date.now() - t0) / 1000).toFixed(1)}s）`);
}
